...
namespace KnobsAsset
{
    ...
    /// </summary>
    public class ShooterKnobSingleListener : KnobListener
    {
        [System.Serializable]
        ...
        [SerializeField] private VariableTypes variableTypes = VariableTypes.DETECT_ANGLE;
        ...
        public override void OnKnobValueChange(float knobPercentValue)
        {
            ...
            switch (variableTypes)
            {
                case VariableTypes.DETECT_ANGLE:
                    gameObjectToManipulate.GetComponent<MMConeOfVision>().VisionAngle = variableValue + (RelativeToInitialValue ? initialDetectAngle : 0);
                    break;
                case VariableTypes.DETECT_RADIUS:
                    gameObjectToManipulate.GetComponent<MMConeOfVision>().VisionRadius = variableValue + (RelativeToInitialValue ? initialDetectRadius : 0);
                    break;
                case VariableTypes.DAMAGE_PER_BULLET:
                    if (playerBulletInstanceDamageOnTouches != null)
                    {
                        ...
                        {
                            bulletDamageOnTouch.GetComponent<DamageOnTouch>().MaxDamageCaused = variableValue + (RelativeToInitialValue ? initialDamagePerBullet : 0);
                            bulletDamageOnTouch.GetComponent<DamageOnTouch>().MinDamageCaused = variableValue + (RelativeToInitialValue ? initialDamagePerBullet : 0);
                        }
                    }
                    break;
                case VariableTypes.BULLET_SPEED:
                    if (playerBulletInstanceProjectiles != null)
                    {
                        ...
                        {
                            bulletProjectile.GetComponent<Projectile>().Speed = variableValue + (RelativeToInitialValue ? initialBulletSpeed : 0);
                        }
                    }
                    break;
                case VariableTypes.MOVE_SPEED:
                    gameObjectToManipulate.GetComponent<CharacterMovement>().MovementSpeed = variableValue + (RelativeToInitialValue ? initialMoveSpeed : 0);
                    break;
                case VariableTypes.SHOOT_INTERVAL:
                    if(playerGunInstance != null) playerGunInstance.GetComponent<ProjectileWeapon>().TimeBetweenUses = variableValue + (RelativeToInitialValue ? initialShootInterval : 0);
                    break;
                default:
                    Debug.LogException(new System.InvalidOperationException("Invalid PositiveVariableTypes value " + variableTypes), this);
                    return;
            }
        }
    }
